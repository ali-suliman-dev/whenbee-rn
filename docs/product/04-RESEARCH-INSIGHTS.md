# 04 — Research Insights: 10x Ideas, Net-New Recs, Open Decisions
...
## 1. The 10x ideas, by build status

### Already built (the heavy lifting is done)
- **Day-1 population priors** — lands the aha in session 1 (where ~50% of conversions/cancels happen). ✅
- **Personal multiplier as the hero artifact** ("you run 1.8× over on writing") — the unoccupied wedge. ✅
...
- **No-streak / no-loss-state design** — the structural contrast vs competitors who ship streaks. ✅

### Net-new / under-weighted (worth adding)
...
### Insight → action bridge (from `.claude/docs/ai/whenbee/10x/session-1.md`, Part 2 — NOT built, NOT in build plan)

Session-1's solo "bridge insight → action" batch never made it into the build plan or the app, yet it answers the doc's own sharpest critique: *knowing your multiplier is an insight; changing behavior is the value, and insight alone churns.* These are the highest-leverage net-new candidates, all on-device, all free.

| Idea | What | Why it matters | Effort |
|---|---|---|---|
...
## 2. Monetization insights beyond the locked model

From `important-docs/Monetization.md`, additive to (not conflicting with) Model B:
- **Paying is an ADHD commitment device.** Non-payers have no skin in the game → inflate churn → make D7 look broken. Free-rider churn is a feature of the free tier, not a bug to fear, but it means the *paid* cohort is the real retention signal.
...
## 3. Self-insight retention doc (08) — still relevant?

**Yes — and largely realized.** Its core thesis (the reward this audience craves is self-understanding, "oh THAT'S why," not a productivity score) is the spine of the Patterns tab, and nearly every mechanic it proposed (S1, S2, S5–S10, plus the Pro correlations S3/S4/S12) is built. Its binding shape — **passive + sparse + on-demand**, never mandatory daily logging — is honored. Its copy guardrails are folded into [06-BRAND-VOICE](06-BRAND-VOICE.md). Treat doc 08 as **fulfilled spec**, not a backlog; revisit only the AI Reflection Coach (S11), which remains a deliberate future add-on.
...
- **Audience:** adults 18–40 with ADHD time-blindness who chronically under-estimate and feel shame about lateness. VOC: "time blindness," "time optimist," "what time do I need to leave," "an app that works with my brain." Never scold.
- **Realistic scale:** ≈ $15–20K ARR Year 1 (~2.5% install→paid on ~15K installs) — a lifestyle product, not venture.

---

## 5. Open founder decisions (the research left these unresolved)
