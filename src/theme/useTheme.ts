...
export function resolveTheme(mode: ColorMode) {
  return { mode, colors: tokens.colors[mode], space: tokens.space, radii: tokens.radii,
    ...
    companion: tokens.companion, ring: tokens.ring, seal: tokens.seal, mote: tokens.mote, row: tokens.row, planRail: tokens.planRail, upsell: tokens.upsell, quick: tokens.quick, chart: tokens.chart, proTeaser: tokens.proTeaser, discovery: tokens.discovery,
    reveal: tokens.reveal, quizComb: tokens.quizComb, honeyGlyph: tokens.honeyGlyph, focusCurve: tokens.focusCurve, headerRing: tokens.headerRing };
}
