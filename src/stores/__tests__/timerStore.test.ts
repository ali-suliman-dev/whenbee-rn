...
// ── setQuickDetails ──────────────────────────────────────────────────────────
// Written as a separate describe so the existing tests stay untouched above.
describe('timerStore — setQuickDetails', () => {
  beforeEach(() => useTimerStore.getState().cancel());

  it('patches taskLabel and category on a running quick-start session', () => {
    useTimerStore.getState().quickStart(T0);
    useTimerStore.getState().setQuickDetails('Send invoices', 'admin');
    const st = useTimerStore.getState();
    expect(st.taskLabel).toBe('Send invoices');
    expect(st.category).toBe('admin');
    // isQuickStart stays true; timer still running; no other state cleared
    expect(st.isRunning).toBe(true);
    expect(st.isQuickStart).toBe(true);
    expect(st.startedAt).toBe(T0);
  });

  it('patches a normal (non-quick-start) session without clearing it', () => {
    useTimerStore.getState().start({ label: 'Emails', category: 'admin', estimateMin: 20 }, T0);
    useTimerStore.getState().setQuickDetails('Renamed', 'cooking');
    const st = useTimerStore.getState();
    expect(st.taskLabel).toBe('Renamed');
    expect(st.category).toBe('cooking');
    expect(st.isRunning).toBe(true);
    expect(st.estimateMin).toBe(20); // unchanged
  });

  it('stop after setQuickDetails returns correct active minutes', () => {
    useTimerStore.getState().quickStart(T0);
    useTimerStore.getState().setQuickDetails('Tidy desk', 'cleaning');
    const { actualMin } = useTimerStore.getState().stop(T0 + 10 * MIN);
    expect(actualMin).toBe(10);
  });

  it('is a no-op when no session is running', () => {
    // No start — store is in CLEARED state. Should not throw.
    expect(() => useTimerStore.getState().setQuickDetails('Foo', 'admin')).not.toThrow();
