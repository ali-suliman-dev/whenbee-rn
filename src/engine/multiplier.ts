...
interface ResolveInput {
  guessMinutes: number;
  category: { mEffective: number; n: number };
  recurring: { mEffective: number; n: number } | null;
}
 ...
 */
export function resolveSuggestion({ guessMinutes, category, recurring }: ResolveInput): CalibrationSummary {
  ...
  return {
    multiplier,
    honestMinutes: honestNumber(guessMinutes, multiplier),
    guessMinutes,
    basis,
    label,
    sampleSize: source.n,
